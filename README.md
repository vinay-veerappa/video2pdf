...
2.  Install the required dependencies:
    ...
    _Note: You may need `ffmpeg` installed on your system for video processing._
...
4.  **Configuration**:
    ...
    - _This is required for advanced transcript cleaning features._

    - _This is required for advanced transcript cleaning features._

5.  **Ollama (Optional - For AI Transcript Cleaning)**:
...
3.  **Curate**: Review the extracted slides in the "Curate Slides" interface.
    ...
    - _Click any slide to toggle its status._
...
You can extract transcripts from YouTube playlists or local directories (recursively) using the dedicated tool:

```bash
# Process a YouTube Playlist
python bulk_transcribe.py "https://www.youtube.com/playlist?list=PL..." -o ./output

# Process a Local Directory (creates transcripts using Whisper)
python bulk_transcribe.py "./my_videos" -o ./output --model base
```

### Features:

- **YouTube Playlists**: Downloads all available transcripts/captions.
- **Local Videos**: Automatically transcribes video/audio files (`.mp4`, `.mp3`, etc.) using OpenAI Whisper.
- **Recursive**: Scans subfolders for local files.
- **Models**: Supports different Whisper model sizes (`tiny`, `base`, `small`, `medium`, `large`).
